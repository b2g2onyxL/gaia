...
(function(exports) {
  ...
  SoundManager.prototype.start = function sm_start() {
    ...
    window.addEventListener('setvibration', this);
    window.addEventListener('offvibration', this);
    ...
    window.addEventListener('stopsoundmanager', this);
  ...
  SoundManager.prototype.stop = function sm_stop() {
    ...
    window.removeEventListener('setvibration', this);
    window.removeEventListener('offvibration', this);
    ...
    window.removeEventListener('stopsoundmanager', this);
...
//ittat
   ...
   dump('#################################this.currentVolume.notification' + this.currentVolume.notification);
   window.asyncStorage.setItem('content.volume', this.currentVolume.notification);

//test ittat
    window.asyncStorage.setItem('content.volume123', 234);
    window.asyncStorage.getItem('content.volume123', function onGettingCachedVolume(value) {
            dump('××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××content.volume123：' + value);
          });
//
  ...
  SoundManager.prototype.handleEvent = function sm_handleEvent(e) {
    dump('###############################SoundManager.prototype.handleEvent  ###############################');
    dump('######################cachedVolume[notification]:' + this.cachedVolume.notification);
    dump('######################currentVolume[notification]:' +  this.currentVolume.notification);
    switch (e.type) {
      case 'volumeup':
	...
	dump('##############################################################unmute');
      ...
      case 'homescreenopened':
        ...
        Service.request('hideCustomDialog');
        break;
      case 'stopsoundmanager':
	dump('#############################################################stopsoundmanager');
        this.stop();
        break;
    }
  };
...
//
      ...
      function iterator(channel) {
        window.asyncStorage.getItem('content.volume',
          function onGettingCachedVolume(value) {
            dump('××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××onGettingCachedVolume：' + value);
            if (!value) {
		...
		//ittat
            self.currentVolume[channel] = value;
	    //var settingObject = {};
	    //settingObject['audio.volume.' + channel] = value;
  ...
  SoundManager.prototype.leaveSilentMode = function sm_leaveSlient(channel,
    ...
    if (!skip_restore &&
      ...
      var settingObject = {};
      var self = this;
      dump('#################leaveSilentMode#####################');
      // At least rollback to 1,
      // otherwise we don't really leave silent mode.
      

      //itatt-test
      this.currentVolume[channel] = (this.cachedVolume[channel] > 0) ? this.cachedVolume[channel] : 1;
      //
